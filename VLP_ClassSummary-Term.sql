...
select av.SchoolYearCode as SchoolYear,av.TermId
	...
	,sum(case av.AssessmentName when 'Letter Names' then ar.Score end) as [Letter Names]	
	,sum(case av.AssessmentName when 'Letter Names' then ar.CutOffScore end) as [Letter Names Cutoff]	
	...
	,sum(case av.AssessmentName when 'Beginning Sounds Expressive' then ar.Score end) as [Beginning Sounds Expressive]
	,sum(case av.AssessmentName when 'Beginning Sounds Expressive' then ar.CutOffScore end) as [Beginning Sounds Expressive Cutoff]
	,sum(case av.AssessmentName when 'Phoneme Blending' then ar.Score end)  as [Phoneme Blending]
	,sum(case av.AssessmentName when 'Phoneme Blending' then ar.CutOffScore end)  as [Phoneme Blending Cutoff]
...
st.AssessmentCount,st.CurrentHighRisk,st.BelowBenchmarkCount,Identified,vw.RiskBand,vwp.AssessmentCount
