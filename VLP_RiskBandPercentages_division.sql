...
(
	...
	AND SA.[TermId]=(case @TermId when 2 then 1 else @TermId end)
),
CTE_Students
AS
(
...
(
	...
	WHERE AV.[TermId]=(case @TermId when 2 then 1 else @TermId end)
...
AS
(
	SELECT [DivisionId], [SiteId], [SchoolYearId], [TermId], [GradeLevelId], [High Risk], [Moderate Risk], [Low Risk]
	FROM 
	(
	...
	(
		COUNT([StudentId])
		FOR [RiskBand] IN ([High Risk], [Moderate Risk], [Low Risk])
	) AS P
),
CTE_RiskBands_Division
AS
(
	SELECT [DivisionId], [SchoolYearId], [TermId], [GradeLevelId], [High Risk], [Moderate Risk], [Low Risk]
	FROM 
	(
...
AS
(
	SELECT CS.[DivisionId], CS.[SiteId], CS.[SchoolYearId], CS.[TermId], CS.[GradeLevelId], CS.[Count], RB.[High Risk], RB.[Moderate Risk], RB.[Low Risk]
...
AS
(
	SELECT CS.[DivisionId], CS.[SchoolYearId], CS.[TermId], CS.[GradeLevelId], CS.[Count], RB.[High Risk], RB.[Moderate Risk], RB.[Low Risk]
...
SELECT	LL.[SiteName] [sortableEntity],
		...
		R.[GradeLevelId],
		R.[Count] AS [Assessed],
		R.[High Risk] AS [HighRisk_Total],
		R.[Moderate Risk] AS [ModRisk_Total],
		R.[Low Risk] AS [LowRisk_Total],
